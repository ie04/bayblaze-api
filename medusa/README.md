...
The local Docker setup mirrors the VPS backend services: Medusa, Postgres, and
Redis run through Docker Compose. Medusa is exposed at `http://localhost:9000`,
Postgres is exposed for host tools at `localhost:5433`, and Redis is exposed at
`localhost:6380`.
...
Inside Docker, Medusa still talks to Postgres at `postgres:5432` and Redis at
`redis:6379`, matching the VPS service layout.

The Medusa container can reach a label printer agent running on your host at
`http://host.docker.internal:4786`, matching the default
`LABEL_PRINTER_AGENT_URL` in the local compose file.
